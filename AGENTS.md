...
- Linux live `<video>` scrubbing and Play/Stop trim controls depend on the GStreamer media framework bundled into the AppImage. Keep `bundle.linux.appimage.bundleMediaFramework` enabled, keep the Linux CI package sets aligned with the required Base, Good, libav, ALSA, and PulseAudio plugins, and retain FFmpeg-generated filmstrip and individual-frame previews as the codec-independent fallback.
