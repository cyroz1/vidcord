...
export type PageDropHandler = (files: readonly File[]) => void;
export type PendingPageDrop = { id: number; files: readonly File[] };

type WebEditorProps = {
  dropHandlerRef: MutableRefObject<PageDropHandler | null>;
  pendingDrop: PendingPageDrop | null;
  onPendingDropHandled: (dropId: number) => void;
};
...
export default function WebEditor({
  ...
  useEffect(() => {
    ...
    acceptFiles(pendingDrop.files);
    onPendingDropHandled(pendingDrop.id);
  }, [acceptFiles, onPendingDropHandled, pendingDrop]);
  ...
  const startExport = useCallback(
    async (retryItemIds?: readonly number[]) => {
