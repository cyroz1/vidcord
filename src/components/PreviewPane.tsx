...
import { cancelPreviewGeneration, getFilmstrip, getPreviewClip, getPreviewFrame } from "../ipc";
...
const PreviewPane = forwardRef<PreviewHandle, Props>(function PreviewPane(
  ...
  // The Linux AppImage bundles WebKitGTK's GStreamer media framework and the
  // plugins needed by this element. Keep the FFmpeg filmstrip/frame path as the
  // codec-independent fallback when direct WebView playback is unavailable.
  const supportsLiveScrubPreview = true;
  useEffect(() => {
    if (videoRef.current) {
